...
class WatchlistState extends State<Watchlist> with RouteAware {
  ...
  void initState() {
    ...
    Future.microtask(() {
      ...
      context.read<tvbloc.WatchlistTvBloc>().add(const tvbloc.FetchTvData());
    });
  }
...
class _WatchlistTv extends StatelessWidget {
  const _WatchlistTv({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: BlocBuilder<tvbloc.WatchlistTvBloc, tvbloc.TvState>(
        builder: (context, state) {
          if (state is tvbloc.LoadingData) {
            return const Center(
              child: CircularProgressIndicator(),
            );
          ...
          } else if (state is tvbloc.ErrorData) {
            return Center(
              child: Text(state.message),
            );
          } else {
